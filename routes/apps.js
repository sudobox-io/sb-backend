...
router.post("/", async (req, res) => {
  try {
    ...
    shell.mkdir("-p", join("/compose", composeConfigFile.data.results.name));
    await writeFile(`${join("/compose", composeConfigFile.data.results.name, "docker-compose.yml")}`, yaml.stringify(config));

    compose.upAll({ cwd: join("/compose", composeConfigFile.data.results.name) }).then(
